bool collides_with_map(Map map, int player_id) {
  ...
  for (int i = 0; i < map.trashcan_list_size; i++) {
    BoundingBox bbox = MeshBoundingBox(map.trashcan_list[i].model.meshes[0]);

    bbox.min = Vector3Add(bbox.min, map.trashcan_list[i].pos);
    bbox.max = Vector3Add(bbox.max, map.trashcan_list[i].pos);
  ...
  for (int i = 0; i < map.masher_list_size; i++) {
    BoundingBox bbox = MeshBoundingBox(map.masher_list[i].model.meshes[0]);

    bbox.min = Vector3Add(bbox.min, map.masher_list[i].pos);
    bbox.max = Vector3Add(bbox.max, map.masher_list[i].pos);
  ...
  for (int i = 0; i < map.chest_list_size; i++) {
    BoundingBox bbox = MeshBoundingBox(map.chest_list[i].model.meshes[0]);

    bbox.min = Vector3Add(bbox.min, map.chest_list[i].pos);
    bbox.max = Vector3Add(bbox.max, map.chest_list[i].pos);
  ...
  for (int i = 0; i < map.delivery_list_size; i++) {
    BoundingBox bbox = MeshBoundingBox(map.delivery_list[i].model.meshes[0]);

    bbox.min = Vector3Add(bbox.min, map.delivery_list[i].pos);
    bbox.max = Vector3Add(bbox.max, map.delivery_list[i].pos);
...
int get_aimed_trashcan(Player *p, Map *map, float *nearest_dist) {
  assert(nearest_dist);
  ...
  *nearest_dist = 999999.0f;
  for (int i = 0; i < map->trashcan_list_size; i++) {
    RayHitInfo hit = GetCollisionRayModel(ray, map->trashcan_list[i].model);
    if (hit.hit) {
...
int get_aimed_delivery(Player *p, Map *map, float *nearest_dist) {
  assert(nearest_dist);
  ...
  *nearest_dist = 999999.0f;
  for (int i = 0; i < map->delivery_list_size; i++) {
    RayHitInfo hit = GetCollisionRayModel(ray, map->delivery_list[i].model);
    if (hit.hit) {
      if (hit.distance < *nearest_dist) {
        nearest = i;
        *nearest_dist = hit.distance;
...
int get_aimed_chest(Player *p, Map *map, float *nearest_dist) {
  assert(nearest_dist);
  ...
  *nearest_dist = 999999.0f;
  for (int i = 0; i < map->chest_list_size; i++) {
    RayHitInfo hit = GetCollisionRayModel(ray, map->chest_list[i].model);
    if (hit.hit) {
      if (hit.distance < *nearest_dist) {
        nearest = i;
        *nearest_dist = hit.distance;
...
int get_aimed_masher(Player *p, Map *map, float *nearest_dist) {
  assert(nearest_dist);
  ...
  *nearest_dist = 999999.0f;
  for (int i = 0; i < map->masher_list_size; i++) {
    RayHitInfo hit = GetCollisionRayModel(ray, map->masher_list[i].model);
    if (hit.hit) {
      if (hit.distance < *nearest_dist) {
        nearest = i;
        *nearest_dist = hit.distance;
